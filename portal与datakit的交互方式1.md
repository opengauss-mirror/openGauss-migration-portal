...
1. 安装部分：
   ...
   Datakit在安装完portal之后，先配置portal_home/config/toolspath.properties中各迁移工具的位置，通过调用java -Dorder=install_mysql_all_migration_tools -Dskip=true -jar portalControl-6.0.0.rc1-exec.jar安装全部迁移工具。
...
2. 运行部分：
   ...
   比如java -Dparameter=value -jar  portalControl-6.0.0.rc1-exec.jar
...
3. 交互方式：
   ...
   java -Dworkspace.id=1 -Dorder=start_plan3 -Dskip=true -jar portalControl-6.0.0.rc1-exec.jar
   ...
   java -Dworkspace.id=1 -Dorder=stop_incremental_migration -Dskip=true -jar portalControl-6.0.0.rc1-exec.jar
