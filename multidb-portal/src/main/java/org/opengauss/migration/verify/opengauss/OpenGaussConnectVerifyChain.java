...
/**
 * OpenGauss connect verify chain
 *
 * @since 2025/7/9
 */
public class OpenGaussConnectVerifyChain extends AbstractOpenGaussVerifyChain {
    private static final Logger LOGGER = LogManager.getLogger(OpenGaussConnectVerifyChain.class);
    private static final String VERIFY_NAME = "OpenGauss Connect Verify";

    @Override
    public void verify(VerifyDto verifyDto, VerifyResult verifyResult) {
        chainResult.setName(VERIFY_NAME);

        try (Connection conn = JdbcUtils.getOpengaussConnection(verifyDto.getTargetIp(), verifyDto.getTargetPort(),
                verifyDto.getTargetDatabase(), verifyDto.getTargetUsername(), verifyDto.getTargetPassword())) {
