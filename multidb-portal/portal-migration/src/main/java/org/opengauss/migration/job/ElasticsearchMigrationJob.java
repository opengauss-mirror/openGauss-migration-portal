...
import org.opengauss.migration.domain.model.MigrationStopIndicator;
import org.opengauss.domain.model.TaskWorkspace;
import org.opengauss.migration.enums.MigrationStatusEnum;
import org.opengauss.migration.exceptions.MigrationException;
import org.opengauss.migration.job.config.ElasticsearchMigrationJobConfig;
...
import org.opengauss.migration.tasks.impl.ElasticsearchMigrationToolFullMigrationTask;
...
/**
 * ElasticsearchMigrationJob
 *
 * @since 2025/10/24
 */
public class ElasticsearchMigrationJob extends AbstractMigrationJob {
    private static final Logger LOGGER = LogManager.getLogger(ElasticsearchMigrationJob.class);

    private final ElasticsearchMigrationJobConfig migrationJobConfig;

    private boolean hasAdjustKernelParam = false;

    public ElasticsearchMigrationJob(ElasticsearchMigrationJobConfig migrationJobConfig) {
    ...
    public boolean preMigrationVerify() {
        return VerifyManager.elasticsearchMigrationVerify(
    ...
    void generateTasks(MigrationStopIndicator migrationStopIndicator, ProcessMonitor processMonitor) {
        ...
        ElasticsearchMigrationConfig migrationConfigDto = migrationJobConfig.getMigrationConfigDto();
        Map<String, String> tableMappings = migrationJobConfig.getTableMappings();
        fullMigrationTask = new ElasticsearchMigrationToolFullMigrationTask(
